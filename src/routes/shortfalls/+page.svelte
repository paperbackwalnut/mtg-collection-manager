<script lang="ts">
	...
	import type { ShortfallCard } from './+page.server';
	...
	// ── Sorting ───────────────────────────────────────────────────────────
	type SortCol = 'name' | 'owned' | 'deckCount' | 'shortfall' | 'price' | 'cost';
	let sortCol = $state<SortCol>('shortfall');
	...
	function setSort(col: SortCol) {
		if (sortCol === col) {
			...
			sortCol = col;
			// Price/cost default to desc (biggest first); name defaults to asc
	...
	// ── Search + basics filter ─────────────────────────────────────────────
	let search = $state('');

	const displayCards = $derived.by(() => {
		let cards = data.cards as ShortfallCard[];
	...
	// ── Summary ───────────────────────────────────────────────────────────
	const totalShortfall = $derived(displayCards.reduce((s, c) => s + c.shortfall, 0));
	const missingCards = $derived(displayCards.filter((c) => c.collQty === 0).length);
	...
	async function addToList(cardName: string, shortfall: number) {
		await addItem(cardName, shortfall, 'shortfalls');
		await refreshList();
	...
	async function addAllVisible() {
		if (bulkBusy) return;
		bulkBusy = true;
		await Promise.all(displayCards.map((c) => addItem(c.cardName, c.shortfall, 'shortfalls')));
		await refreshList();
		bulkBusy = false;
...
<div class="page-header">
	<div>
		<h1 class="page-title">Shortfalls</h1>
		<p class="page-subtitle">
			{#if displayCards.length > 0}
				<span style="color:var(--warning);font-weight:600"
					>{displayCards.length} card{displayCards.length !== 1 ? 's' : ''}</span
				>
				where deck usage exceeds owned copies ·
				<span style="color:var(--danger)">{missingCards} with zero owned</span>
				· {totalShortfall} total copies short
				{#if totalCost > 0}· est. low <strong>${totalCost.toFixed(2)}</strong> to fill{/if}
			{:else if ignoreBasics && data.cards.length > 0}
				All shortfalls are basic lands (currently hidden)
			{:else}
				All card quantities are covered by your collection
			{/if}
		</p>
	</div>
	<div class="flex gap-2">
		<a href="/help#planning" class="btn btn-sm">How this works</a>
...
{#if data.cards.length === 0}
	<div class="empty-state">
		<p>No shortfalls — your collection covers all deck copy requirements.</p>
		...
		<thead>
			<tr>
				...
				<th
					...
					style="cursor:pointer"
				>
					Decks {sortIcon('deckCount')}
				</th>
				<th
					class="sortable"
					onclick={() => setSort('shortfall')}
					title="Shortfall = total copies needed − owned"
					style="cursor:pointer;color:var(--warning)"
				>
					Short {sortIcon('shortfall')}
				</th>
				<th
					class="sortable"
					onclick={() => setSort('price')}
					title="Lowest non-null Scryfall USD price across all printings (local cache) — estimate only"
					style="cursor:pointer"
				>
					Low Price {sortIcon('price')}
				</th>
				<th
					class="sortable"
					onclick={() => setSort('cost')}
					title="Est. Cost = lowest price × shortfall — estimate for acquisition planning, not a specific printing price"
					style="cursor:pointer"
				>
		...
		<tbody>
			{#each displayCards as card}
				<tr>
					...
					<td
						style="font-weight:600;font-size:13px;color:{card.collQty === 0
							? 'var(--danger)'
							: 'var(--text-muted)'}"
					>
						{card.collQty}
					...
					<td>
						<span
							style="font-weight:700;font-size:13px;color:{card.collQty === 0
								? 'var(--danger)'
								: 'var(--warning)'}"
						>
							−{card.shortfall}
					...
					<td style="white-space:nowrap">
						...
						{:else}
							<button
								...
								onclick={() => addToList(card.cardName, card.shortfall)}
